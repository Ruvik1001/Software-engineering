...
@calendar_proxy_router.get("/events", summary="Proxy: list calendar events", description="Authorized route. Calls mock calendar service.")
async def calendar_events(token: str = Depends(get_token_value), use_case: CalendarProxyUseCase = Depends(get_calendar_proxy_use_case)):
